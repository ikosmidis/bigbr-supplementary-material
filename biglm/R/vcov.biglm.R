getR <- function(object) {
    if (!isTRUE(object$checked)) {
        object <- singcheck.bigqr(object)
    }
    D <- object$D
    Rbar <- diag(length(D))
    Rbar[lower.tri(Rbar)] <- object$rbar
    sqrt(D) * t(Rbar)
}

vcov.biglm <- function(object, ...) {
...
vcov.bigglm <- function(object, dispersion = NULL, ...) {
    p <- length(object$qr$D)
    R <- getR(object$qr)
    ok <- object$qr$D != 0
